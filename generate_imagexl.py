...
def worker(device, prompt, output_file, finish_event, iterations=20, model="stabilityai/stable-diffusion-2-1"):
	...
	try:
		if not device=="cpu":
			...
			if major >= 6:
				pipe = StableDiffusionPipeline.from_pretrained(pretrained_model_name_or_path=savedmodel, safety_checker=None, torch_dtype=torch.float16, variant="fp16", use_safetensors=False, low_cpu_mem_usage=False, device_map=None)
			else:
